...
<template>
    <div @click="[resetMenu(), $emit('toggleMenu')]" v-if="isCategoryMenuShowing" v-motion-fade
        class="fixed w-screen h-full inset-0 bg-black/10 z-[998]">
    </div>
    <div v-if="isCategoryMenuShowing"
        class="absolute  rounded-3xl top-[150%] right-0 flex flex-col divide-y divide-neutral-100 md:divide-neutral-200 shadow text-sm md:text-base menu z-[999]"
        ...
        <div v-for="category, index in categories" :key="index"
            ...
            <div class="flex items-center w-full justify-between px-5 absolute left-2/4 -translate-x-2/4 transition-all duration-350 "
                ...
                <button @click="toggleCategoryClicked(index)"
                    ...
                    <div class=" grid place-items-center rounded-full badge">
                        <p class="text-sm md:text-base select-none" :style="{ color: category.categoryColor }">{{
