...
const toggleVisibility = () => {
  ...
  passwordInputType.value =
    passwordInputType.value == "text" ? "password" : "text";
};
...
const { errors, meta, defineField, isSubmitting, handleSubmit } =
...
const register = handleSubmit((values) => {
  fetch("http://localhost:4321/v1/users/register", {
    method: "POST",
    ...
    .then((response) => {
      if (response.ok) {
        ...
        return response.json().then((data) => {
          const t = useLocalStorage("token", data.token);
        });
      }
      return response.json().then((data) => {
        ...
        if (errorType == "unique violation") {
          toast.error("Oops! Email's taken. Pick another");
        }
      });
    })
    .catch((error) => {});
...
<template>
  <main
    class="flex flex-col gap-8 justify-center items-center p-6 max-w-4xl m-auto"
  >
    ...
    <h1 class="font-gelion_medium text-lg w-full max-w-sm lg:text-3xl">
      Create an account
    </h1>
    ...
    >
      <div class="flex flex-col gap-1 items-start">
        ...
        <small v-show="errors.email" class="pl-1 text-red-500">{{
          errors.email
        }}</small>
        <div class="relative w-full">
          ...
          >
            <Check
              stroke-width="3"
              class="stroke-green-700 size-[10px] lg:size-3"
            />
          </div>
        </div>
      </div>
      <div class="flex flex-col gap-1 items-start">
        ...
        <small v-show="errors.password" class="pl-1 text-red-500">{{
          errors.password
        }}</small>
        <div class="relative w-full">
          <input
          ...
          <div
            ...
            class="absolute top-2/4 right-0 -translate-y-2/4 p-4 cursor-pointer"
          >
            <Eye v-if="isPasswordVisible" class="stroke-neutral-500 size-4" />
            <EyeOff
              v-if="!isPasswordVisible"
              class="stroke-neutral-500 size-4"
            />
          </div>
        </div>
      </div>
    ...
    <div class="px-6 relative flex w-full">
      <div
        class="absolute left-0 h-[1px] w-full bg-neutral-200/60 top-2/4 -translate-y-2/4"
      ></div>
      <small
        class="text-center relative z-10 bg-white px-3 max-w-fit m-auto lg:text-lg"
        >or</small
      >
    </div>
    <a
